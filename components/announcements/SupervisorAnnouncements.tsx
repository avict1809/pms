...
interface SupervisorAnnouncementsProps {
  className?: string;
  maxItems?: number;
}
...
export default function SupervisorAnnouncements({
  ...
  const fetchAnnouncements = async () => {
    try {
      ...
      if (response.ok) {
        // Filter announcements relevant to supervisors
        const supervisorAnnouncements = result.data.filter(
          (announcement: Announcement) => {
            return (
              announcement.target_type === "global" ||
              announcement.target_type === "supervisor" ||
              (announcement.target_type === "supervisor" &&
        ...
        setAnnouncements(supervisorAnnouncements);
      } else {
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-neutral-400 mt-1">
            Stay updated with announcements relevant to supervisors
          </p>
        </div>
        <Button
          variant="outline"
        ...
        {filteredAnnouncements.length > 0 ? (
          ...
          <Card className="bg-[#23232a] border-orange-500 shadow-lg">
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-white mb-2">
                No announcements for supervisors
              </h3>
              <p className="text-neutral-400">
                {searchTerm || filterType !== "all"
                  ? "Try adjusting your search or filters"
                  : "No supervisor-specific announcements have been posted yet"}
              </p>
            </CardContent>
          </Card>
        )}
