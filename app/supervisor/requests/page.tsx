...
export default function SupervisorRequestsPage() {
  ...
  // Fetch all projects for dropdown and for filtering
  useEffect(() => {
  ...
  // Fetch requests for the supervisor
  useEffect(() => {
    async function fetchRequests() {
      ...
      // 1. Get all projects where supervisor is assigned
      const { data: supervisedProjects, error: projError } = await supabase
        .from("projects")
        .select("id")
        .eq("supervisor_id", user.id);
      const supervisedProjectIds =
        supervisedProjects?.map((p: any) => p.id) || [];
      // 2. Get requests made by supervisor
      const { data: ownRequests, error: ownError } = await supabase
        .from("approval_requests")
        .select("*")
        .eq("requested_by", user.id);
      // 3. Get requests for supervised projects
      let projectRequests: any[] = [];
      if (supervisedProjectIds.length > 0) {
        const { data: projReqs, error: projReqsError } = await supabase
          .from("approval_requests")
          .select("*")
          .in("project_id", supervisedProjectIds);
        projectRequests = projReqs || [];
      }
      // 4. Merge and deduplicate
      const allRequests = [...(ownRequests || []), ...projectRequests].filter(
  ...
  // Handle form submit (supervisors can make requests)
